...
Esw_on=41;     %mJ - IGBT's turn-on switching energy per pulse at peak current (Icp, T=125C)
Esw_off=32;    %mJ - IGBT's turn-off switching energy per pulse at peak current (Icp, T=125C)
...
Vec = ;        %V - FWD forward voltage drop @Iep
D=  ;           % - PWM duty factor (Modulation depth)
phase_angle=18.195; %degree - phase angle btw. output voltage and current pf=0.95
Irr=  ;           % - Diode peak recovery current
trr= 300;       %ns - Diode reverse recovery time
Vce_peak=  ;       % - Peak voltage across th diode at recovery
...
Prr=0.125*Irr*trr*Vce_peak*fsw;                  %Switcihng Loss per
